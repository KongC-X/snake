...
Snake.prototype.strategies = {
    move:function (format){   //这个参数决定要不要删除最后一个方块，也就是蛇尾。传了参数就是吃
        //创建新身体（在旧蛇头的位置）
...
function createFood() { 
    ...
    var include = true;  //循坏跳出的条件，为true表示食物坐标在蛇身上，继续循坏，为false表示食物坐标不在蛇身上，跳出循环
    while(include){
...
Game.prototype.init = function () { 
    ...
    document.onkeydown = function (ev) { 
        if(ev.which == 37 && snake.direction != snake.directionNum.right){  //用户按下左键时，蛇不能正在往右走
            snake.direction = snake.directionNum.left;
        }else if(ev.which == 38 && snake.direction != snake.directionNum.down){  //用户按下左键时，蛇不能正在往右走
            snake.direction = snake.directionNum.up;
        }else if(ev.which == 39 && snake.direction != snake.directionNum.left){  //用户按下左键时，蛇不能正在往右走
            snake.direction = snake.directionNum.right;
        }else if(ev.which == 40 && snake.direction != snake.directionNum.up){  //用户按下左键时，蛇不能正在往右走
